...
function buildEmotionSupport(emotion: EmotionType, companion: CompanionType): string {
  ...
  switch (emotion) {
    case 'sad':
      return `${prefix} 这份难受我接住了。你不用急着讲完整，先说最压着你的那一下也可以。`;
    case 'anxious':
      return `${prefix} 你现在像是一直绷着。先不用马上把所有问题解开，我们先把最乱的那一团放下来一点。`;
    case 'angry':
      return `${prefix} 会这么烦，通常是心里已经憋了很久。你可以先把最让你堵得慌的那一句说出来。`;
    case 'happy':
      return `${prefix} 这份轻一点的感觉也值得被好好看见。你想从哪一小段开始讲都可以。`;
    case 'calm':
      return `${prefix} 现在这个状态很适合慢慢说。你想先讲讲最近哪件事最占心里位置？`;
    default:
      return `${prefix} 你可以不用整理好再说，从最想先讲的那一句开始就行。`;
  }
}
